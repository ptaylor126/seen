...
- Write the remaining migrations for TECHNICAL §1 (`friendships`, `friend_requests`, `recommendations`, `notifications`, `push_tokens`). For each: enable RLS, write SELECT/INSERT/UPDATE/DELETE policies, then run the `rls-auditor` subagent before applying. `friendships` should be next so `is_friend_of_auth()` becomes executable.
